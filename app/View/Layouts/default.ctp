...
<body>
    <div class="script-security">
        <script>
            if ( window.location === window.parent.location && window.location.toString().indexOf('users/login') > -1 ) {
                document.getElementsByTagName('body')[0].innerHTML = '';
                location.href = "https://dev.ecrimelabs.net:7000/misp/login";
            } else {
                document.getElementsByClassName('script-security')[0].innerHTML = '';
            }
        </script>
    </div>
    ...
    <script>
        window.addEventListener('message', function(event) {
            ...
            if (~event.origin.indexOf(baseurl + ':7000')) {
                if (event.data == 'message') {
                    var key = $('input[name=data\\[_Token\\]\\[key\\]]').val();
                    console.log(key);
